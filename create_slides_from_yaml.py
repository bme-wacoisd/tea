"""
Generate Google Slides presentations from YAML configuration files.

USAGE:
    python create_slides_from_yaml.py lessons/01-james-baldwin-civil-rights/slides.yaml
...
class SlideBuilder:
    ...
    def create_presentation(self):
        ...
        # Delete the default template slide
    ...
    def build_title_slide(self, slide_id, slide_config):
        ...
        # Header bar
        self.add_shape(requests, slide_id, self.make_id('header'), 'RECTANGLE', 0, 0, 13.333, 2.5, self.colors['primary'])
        # Gold stripe
        self.add_shape(requests, slide_id, self.make_id('stripe'), 'RECTANGLE', 0, 2.4, 13.333, 0.15, self.colors['accent1'])
        # Title
        self.add_shape(requests, slide_id, self.make_id('title'), 'TEXT_BOX', 0.5, 0.6, 12.333, 1.5, None)
        self.add_text(requests, self.make_id('title'), title, font_size=48, bold=True, color=self.colors['white'], alignment='CENTER')
        # Subtitle
        if subtitle:
            self.add_shape(requests, slide_id, self.make_id('subtitle'), 'TEXT_BOX', 0.5, 3.2, 12.333, 0.8, None)
            self.add_text(requests, self.make_id('subtitle'), subtitle, font_size=28, color=self.colors['text'], alignment='CENTER')
        # Decorative circle
        self.add_shape(requests, slide_id, self.make_id('circle'), 'ELLIPSE', 11, 5.5, 1.5, 1.5, self.colors['accent2'])
    ...
    def build_big_idea_slide(self, slide_id, slide_config):
        ...
        self.add_shape(requests, slide_id, self.make_id('bg'), 'RECTANGLE', 0, 0, 13.333, 7.5, self.colors['primary'])
        self.add_shape(requests, slide_id, self.make_id('accent'), 'RECTANGLE', 0, 0, 0.3, 7.5, self.colors['accent1'])
        self.add_shape(requests, slide_id, self.make_id('label'), 'TEXT_BOX', 0.8, 0.5, 11, 1, None)
        self.add_text(requests, self.make_id('label'), label, font_size=24, bold=True, color=self.colors['accent1'])
        self.add_shape(requests, slide_id, self.make_id('main'), 'TEXT_BOX', 0.8, 1.8, 11.5, 2, None)
        self.add_text(requests, self.make_id('main'), main_text, font_size=56, bold=True, color=self.colors['white'])
        if supporting:
            self.add_shape(requests, slide_id, self.make_id('support'), 'TEXT_BOX', 0.8, 4.5, 10, 1.5, None)
            self.add_text(requests, self.make_id('support'), supporting, font_size=28, color=self.colors['background'])
    ...
    def build_image_bio_slide(self, slide_id, slide_config):
        ...
        self.add_shape(requests, slide_id, self.make_id('header'), 'RECTANGLE', 0, 0, 13.333, 1.3, self.colors['secondary'])
        self.add_shape(requests, slide_id, self.make_id('title'), 'TEXT_BOX', 0.5, 0.3, 12, 0.8, None)
        self.add_text(requests, self.make_id('title'), title, font_size=36, bold=True, color=self.colors['white'])

        if image_key:
            image_url = self.upload_image(image_key)
            if image_url:
                self.add_image(requests, slide_id, self.make_id('photo'), image_url, 0.8, 1.8, 3.5, 4)
            else:
                self.add_shape(requests, slide_id, self.make_id('photo'), 'ROUND_RECTANGLE', 0.8, 1.8, 3.5, 4, self.colors['text'])

        self.add_shape(requests, slide_id, self.make_id('desc'), 'TEXT_BOX', 5, 2, 7.5, 4, None)
        self.add_text(requests, self.make_id('desc'), description, font_size=32, color=self.colors['text'])
        self.add_shape(requests, slide_id, self.make_id('bottom'), 'RECTANGLE', 0, 7.2, 13.333, 0.3, self.colors['accent1'])
    ...
    def build_quote_slide(self, slide_id, slide_config):
        ...
        self.add_shape(requests, slide_id, self.make_id('bg'), 'RECTANGLE', 0, 0, 13.333, 7.5, self.colors['text'])
        self.add_shape(requests, slide_id, self.make_id('qmark'), 'TEXT_BOX', 0.5, 0.5, 2, 2, None)
        self.add_text(requests, self.make_id('qmark'), '"', font_size=144, bold=True, color=self.colors['accent1'])
        self.add_shape(requests, slide_id, self.make_id('quote'), 'TEXT_BOX', 1.5, 2, 10.5, 3.5, None)
        self.add_text(requests, self.make_id('quote'), quote, font_size=36, color=self.colors['white'])
        if attribution:
            self.add_shape(requests, slide_id, self.make_id('attr'), 'TEXT_BOX', 1.5, 5.8, 10, 1, None)
            self.add_text(requests, self.make_id('attr'), f'— {attribution}', font_size=24, color=self.colors['accent1'])
    ...
    def build_table_slide(self, slide_id, slide_config):
        ...
        self.add_shape(requests, slide_id, self.make_id('header'), 'RECTANGLE', 0, 0, 13.333, 1.3, accent)
        self.add_shape(requests, slide_id, self.make_id('title'), 'TEXT_BOX', 0.5, 0.3, 12, 0.8, None)
        self.add_text(requests, self.make_id('title'), title, font_size=32, bold=True, color=self.colors['white'])

        num_rows = len(rows) + 1
        num_cols = len(headers)
        table_id = self.make_id('table')
        self.create_table(requests, slide_id, table_id, num_rows, num_cols, 0.5, 1.6, 12.333, 5.2)

        for col, header in enumerate(headers):
            self.style_table_cell(requests, table_id, 0, col, text=header, bg_color=accent, text_color=self.colors['white'], bold=True, font_size=16)

        for row_idx, row_data in enumerate(rows):
            bg = self.colors['background'] if row_idx % 2 == 0 else self.colors['white']
            for col_idx, cell_text in enumerate(row_data):
                self.style_table_cell(requests, table_id, row_idx + 1, col_idx, text=cell_text, bg_color=bg, text_color=self.colors['text'], font_size=14)
    ...
    def build_comparison_slide(self, slide_id, slide_config):
        ...
        self.add_shape(requests, slide_id, self.make_id('header'), 'RECTANGLE', 0, 0, 13.333, 1.3, self.colors['primary'])
        self.add_shape(requests, slide_id, self.make_id('title'), 'TEXT_BOX', 0.5, 0.3, 12, 0.8, None)
        self.add_text(requests, self.make_id('title'), title, font_size=32, bold=True, color=self.colors['white'])

        self.add_shape(requests, slide_id, self.make_id('left_hdr'), 'ROUND_RECTANGLE', 0.5, 1.6, 5.9, 0.8, self.colors['secondary'])
        self.add_text(requests, self.make_id('left_hdr'), left_header, font_size=22, bold=True, color=self.colors['white'], alignment='CENTER')

        self.add_shape(requests, slide_id, self.make_id('right_hdr'), 'ROUND_RECTANGLE', 6.9, 1.6, 5.9, 0.8, self.colors['accent2'])
        self.add_text(requests, self.make_id('right_hdr'), right_header, font_size=22, bold=True, color=self.colors['white'], alignment='CENTER')

        left_text = '\n'.join([f'✓  {item}' for item in left_items])
        self.add_shape(requests, slide_id, self.make_id('left_box'), 'TEXT_BOX', 0.7, 2.6, 5.5, 4.5, None)
        self.add_text(requests, self.make_id('left_box'), left_text, font_size=18, color=self.colors['text'])

        right_text = '\n'.join([f'✗  {item}' for item in right_items])
        self.add_shape(requests, slide_id, self.make_id('right_box'), 'TEXT_BOX', 7.1, 2.6, 5.5, 4.5, None)
        self.add_text(requests, self.make_id('right_box'), right_text, font_size=18, color=self.colors['text'])
    ...
    def build_two_column_slide(self, slide_id, slide_config):
        ...
        self.add_shape(requests, slide_id, self.make_id('left_title'), 'TEXT_BOX', 0.5, 1.6, 5.5, 0.7, None)
        self.add_text(requests, self.make_id('left_title'), left_title, font_size=22, bold=True, color=self.colors['primary'])
        left_text = '\n'.join([f'•  {b}' for b in left_items])
        self.add_shape(requests, slide_id, self.make_id('left_box'), 'TEXT_BOX', 0.5, 2.4, 5.5, 4.5, None)
        self.add_text(requests, self.make_id('left_box'), left_text, font_size=18, color=self.colors['text'])

        self.add_shape(requests, slide_id, self.make_id('right_title'), 'TEXT_BOX', 7, 1.6, 5.5, 0.7, None)
        self.add_text(requests, self.make_id('right_title'), right_title, font_size=22, bold=True, color=self.colors['primary'])
        right_text = '\n'.join([f'•  {b}' for b in right_items])
        self.add_shape(requests, slide_id, self.make_id('right_box'), 'TEXT_BOX', 7, 2.4, 5.5, 4.5, None)
        self.add_text(requests, self.make_id('right_box'), right_text, font_size=18, color=self.colors['text'])
    ...
    def build_bullets_slide(self, slide_id, slide_config):
        ...
        for i, bullet in enumerate(items):
            self.add_shape(requests, slide_id, self.make_id(f'dot_{i}'), 'ELLIPSE', 0.6, y_pos + 0.15, 0.35, 0.35, accent)
            self.add_shape(requests, slide_id, self.make_id(f'txt_{i}'), 'TEXT_BOX', 1.2, y_pos, 11, 0.8, None)
            self.add_text(requests, self.make_id(f'txt_{i}'), bullet, font_size=22, color=self.colors['text'])
            y_pos += 1.0
    ...
    def build_numbered_slide(self, slide_id, slide_config):
        ...
        for i, item in enumerate(items):
            self.add_shape(requests, slide_id, self.make_id(f'circle_{i}'), 'ELLIPSE', 0.5, y_pos, 0.7, 0.7, colors_cycle[i % len(colors_cycle)])
            self.add_shape(requests, slide_id, self.make_id(f'num_{i}'), 'TEXT_BOX', 0.5, y_pos + 0.1, 0.7, 0.5, None)
            self.add_text(requests, self.make_id(f'num_{i}'), str(i + 1), font_size=24, bold=True, color=self.colors['white'], alignment='CENTER')
            self.add_shape(requests, slide_id, self.make_id(f'item_{i}'), 'TEXT_BOX', 1.5, y_pos + 0.1, 10.5, 0.8, None)
            self.add_text(requests, self.make_id(f'item_{i}'), item, font_size=24, color=self.colors['text'])
            y_pos += 1.2
    ...
    def build_closing_slide(self, slide_id, slide_config):
        ...
        self.add_shape(requests, slide_id, self.make_id('bg'), 'RECTANGLE', 0, 0, 13.333, 7.5, self.colors['primary'])
        self.add_shape(requests, slide_id, self.make_id('deco1'), 'ELLIPSE', -0.5, -0.5, 3, 3, self.colors['secondary'])
        self.add_shape(requests, slide_id, self.make_id('deco2'), 'ELLIPSE', 11, 5.5, 3, 3, self.colors['accent2'])

        self.add_shape(requests, slide_id, self.make_id('quote'), 'TEXT_BOX', 1.5, 1.5, 10.5, 3, None)
        self.add_text(requests, self.make_id('quote'), f'"{quote}"', font_size=32, color=self.colors['white'], alignment='CENTER')
        if attribution:
            self.add_shape(requests, slide_id, self.make_id('attr'), 'TEXT_BOX', 1.5, 4.8, 10.5, 0.8, None)
            self.add_text(requests, self.make_id('attr'), f'— {attribution}', font_size=24, bold=True, color=self.colors['accent1'], alignment='CENTER')
        if cta:
            self.add_shape(requests, slide_id, self.make_id('cta'), 'TEXT_BOX', 1.5, 6.2, 10.5, 0.8, None)
            self.add_text(requests, self.make_id('cta'), cta, font_size=20, color=self.colors['background'], alignment='CENTER')
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python create_slides_from_yaml.py <path_to_slides.yaml>")
        print("Example: python create_slides_from_yaml.py lessons/01-james-baldwin-civil-rights/slides.yaml")
        sys.exit(1)
